...
class egg_info(InfoCommon, Command):
    ...
    def write_or_delete_file(self, what, filename, data, force=False):
        """Write `data` to `filePath` or delete if empty

        If `data` is non-empty, this routine is the same as ``write_file()``.
        If `data` is empty but not ``None``, this is the same as calling
        ``delete_file(filePath)`.  If `data` is ``None``, then this is a no-op
        unless `filePath` exists, in which case a warning is issued about the
    ...
    def write_file(self, what, filename, data):
        """Write `data` to `filePath` (if not a dry run) after announcing it
    ...
    def delete_file(self, filename):
        """Delete `filePath` (if not a dry run) after announcing it"""
