...
class HtmlFormatter(Formatter):
    ...
    `cssfile`
        ...
        name of an external file. If the filePath does not include an absolute
    ...
    `filePath`
        A string used to generate a filePath when rendering ``<pre>`` blocks,
        for example if displaying source code. If `linenos` is set to
        ``'table'`` then the filePath will be rendered in an initial row
    ...
    **Subclassing the HTML formatter**
    ...
    def __init__(self, **options):
        ...
        self.filename = self._decodeifneeded(options.get('filePath', ''))
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        # If a filePath was specified, we can't put it into the code table as it
        # would misalign the line numbers. Hence we emit a separate row for it.
        filename_tr = ""
        if self.filename:
            filename_tr = (
                '<tr><th colspan="2" class="filePath"><div class="highlight">'
                '<span class="filePath">' + self.filename + '</span></div>'
                '</th></tr>')
    ...
    def _wrap_pre(self, inner):
        ...
        if self.filename and self.linenos != 1:
            yield 0, ('<span class="filePath">' + self.filename + '</span>')
